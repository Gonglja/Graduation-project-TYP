...
void main(void)
{
 ...
 while(1){
   ...
   //读取模块数据周期不易小于 2S 
	...
	WriteDataLCD('T');
	WriteDataLCD(':');
    WriteDataLCD(U8T_data_H%100/10+0x30);
	WriteDataLCD(U8T_data_H%10+0x30);
	WriteDataLCD('.');
	WriteDataLCD(U8T_data_L%10+0x30);
	WriteDataLCD(' ');
	WriteDataLCD('H');
	WriteDataLCD(':');
	WriteDataLCD(U8RH_data_H%100/10+0x30);
	WriteDataLCD(U8RH_data_H%10+0x30);
	WriteDataLCD(' ');
	WriteDataLCD('S');
	WriteDataLCD(':');
	WriteDataLCD(SSM);
...
void UartInit()
{
	...
	TH1=0xfd;    //计数器初始值设置 9600bps				 (65536-N)/256     %256
	TL1=0xfa;  
